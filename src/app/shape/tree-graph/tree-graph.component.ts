...
export class TreeGraphComponent implements OnInit {
  ...
  prepareDataForGraph(formStateMachine: FormStateMachine): TreeNode {
    const stateNames = (vformStateMachine: FormStateMachine) => {
      if (!vformStateMachine.switcher) {
        return null;
      }

      return Object.keys(vformStateMachine.switcher.states).map( state => {
        return {
          name: state,
          children: stateNames(vformStateMachine.switcher.states[state])
        }
      })
    }
